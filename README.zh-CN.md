...
- 响应式 Next.js 工作台、主要标签中英文切换、Electron 开发壳，以及 unsigned Windows x64 安装包构建。
- 后端回归、策略、桌面、仓库卫生和 CI 检查。

## 现在的边界

MemWeave 目前是本地单用户 MVP。核心流程可以运行，适合本地使用、开发和项目展示，但它还不是可直接上线的公网服务，也不是签名后的生产级桌面应用。

- 外部 AI、OCR、ASR 和声音生成默认关闭或未配置。
- FastAPI 后端没有生产级鉴权和多租户隔离，不能直接暴露到公网。
- 默认使用本地 JSON。可选 SQLite 保存的仍是同类文档数据，不是多用户关系型设计。
- v0.2.0 Windows 安装包会携带 Electron、Next.js standalone server 和 FastAPI 后端运行时。它没有代码签名，也没有自动更新。
...
v0.2.0 Release 包含 unsigned Windows x64 `Setup.exe`。由于没有代码签名，Windows 可能显示“未知发布者”警告。

安装版会把数据、设置、日志、附件和生成音频保存在 `%LOCALAPPDATA%\MemWeave`。卸载前请先在“设置”中导出完整备份：卸载时会显示数据删除警告，并默认删除该目录。备份 ZIP 没有加密，需按敏感数据保管。

安装包不包含 IndexTTS2 权重、声音参考、API Key 或个人记忆数据。安装、备份/导入、卸载、可选声音配置和可复现构建命令见 [Windows 安装指南](docs/windows-installer.zh-CN.md)。
...
这两条命令会在 Electron 中启动当前源码检出。详情见 [桌面壳说明](docs/desktop-shell.zh-CN.md)；构建安装包请阅读 [Windows 安装指南](docs/windows-installer.zh-CN.md)。
...
desktop/   Electron 开发壳和安装版运行时启动器
